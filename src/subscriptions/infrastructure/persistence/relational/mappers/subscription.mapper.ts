...
export class SubscriptionMapper {
    static toDomain(raw: SubscriptionEntity): Subscription {
        ...
        domain.plan = raw.plan
            ? SubscriptionPlanMapper.toDomain(raw.plan)
            : null;

        return domain;
    }

    static toPersistence(domain: Partial<Subscription>): Partial<SubscriptionEntity> {
        const entity: Partial<SubscriptionEntity> = {};

        if (domain.id !== undefined) entity.id = domain.id;
        if (domain.start_date !== undefined) entity.start_date = domain.start_date;
        if (domain.expiry_date !== undefined) entity.expiry_date = domain.expiry_date;
